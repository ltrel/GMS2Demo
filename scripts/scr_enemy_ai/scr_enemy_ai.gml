...
function player_visible() {
	...
	var invis_collider_hit = collision_line(x, y, player.x, player.y, obj_invisible_collider, false, true);
	var door_hit = collision_line(x, y, player.x, player.y, obj_door, false, true);
	var closed_door_hit = door_hit && (!door_hit.is_open);
	return !wall_1_hit && !wall_2_hit && !closed_door_hit && !invis_collider_hit;
}
